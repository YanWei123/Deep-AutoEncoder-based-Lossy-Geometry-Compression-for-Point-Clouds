...
def draw_point_cloud(input_points, canvasSize=500, space=200, diameter=25,
    ...
    M = euler2mat(zrot, yrot, xrot)      # M is the returned extrinsic rotation matrix
    points = (np.dot(M, points.transpose())).transpose()        # need transpose
    ...
    zorder = np.argsort(points[:, 2])       # ascending order
    ...
    for i in range(points.shape[0]):
        ...
        px = dx + xc        # xc 代表高斯圆盘的位置，dx 代表高斯圆盘的坐标，且注意dx是一个小矩阵，而xc是一个标量！
        py = dy + yc
