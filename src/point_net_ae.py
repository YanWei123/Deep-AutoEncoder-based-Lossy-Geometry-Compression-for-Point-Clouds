...
from .. external.quantization.tf_quantization import quantization
...
class PointNetAutoEncoder(AutoEncoder):
    ...
    def __init__(self, name, configuration, graph=None):
        ...
        with tf.variable_scope(name):
            ...
            if c.training:
                ...
                self.z , self.likelihoods = self.entropy_bottleneck(self.z, training=True)
            else:
                # self.z = quantization(self.z)
                self.entropy_bottleneck = tfc.EntropyBottleneck(optimize_integer_offset=False)
                self.z , self.likelihoods = self.entropy_bottleneck(self.z, training=False)
    ...
    def _create_loss(self):
        ...
        train_bpp = tf.reduce_sum(tf.log(self.likelihoods)) / (-np.log(2) * c.n_input[0])
        ...
        self.loss = c.lmbda*self.loss + c.lmbda2*train_bpp
    ...
    def _setup_optimizer(self):
        ...
        self.aux_optimizer = tf.train.AdamOptimizer(learning_rate=1e-3)
        self.aux_step = self.aux_optimizer.minimize(self.entropy_bottleneck.losses[0])

        self.train_step = tf.group(self.train_step,self.aux_step,self.entropy_bottleneck.updates[0])
